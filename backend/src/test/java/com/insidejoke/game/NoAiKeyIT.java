...
class NoAiKeyIT extends AbstractIntegrationTest {
    ...
    void withoutAnAiKeyTheIntakeWorksAndSecretsAreNotOffered() {
        try (Party party = Party.create(port, json, FAKE, 3)) {
            ...
            JsonNode lobby = p.getSocket().latest();
            ...
            assertThat(party.screen.latest().path("secretsOpen").asBoolean(true))
                    .isFalse();
            assertThat(p.getSocket().error("dossier.add", Map.of("text", "A harmless story")))
                    .isEqualTo("MODERATION_UNAVAILABLE");

            party.captain().getSocket().ok("game.start", Map.of());
            p.getSocket().phase("INTAKE");
            JsonNode blocked = p.getSocket()
                    .reply(p.getSocket()
                            .send(
                                    "intake.submit",
            ...
            p.getSocket().ok("intake.submit", Map.of("answers", List.of("Pizza", "Dancing", "Karaoke")));
